...
def chorus():
    with schemdraw.Drawing(file=out("dsp_chorus.svg"), show=False) as d:
        ...
        b = d.add(
            dsp.Box(w=2.8, h=1.4)
            .anchor("W")
            .label("z⁻ᴰ⁽ᵗ⁾", loc="top", ofst=0.2)
            .label("short tap (BRAM)\n~9–17 ms")
        )
...
def svf():
    with schemdraw.Drawing(file=out("dsp_svf.svg"), show=False) as d:
        ...
        f1 = d.add(dsp.Amp().at(hp.center).right().label("× f", "top", ofst=0.08))
        ...
        f2 = d.add(dsp.Amp().at(bp.center).right().label("× f", "top", ofst=0.08))
