...
// ---------- SETTINGS ▸ Firmware ----------
// "Is the board I flashed last month current?" -- and until #27 nothing could answer it, because
// nothing on the wire said what was flashed. It does now, and it is worth being precise about how
// little that took: the Tiliqua's build stamp rides in its USB **iManufacturer** string, which Web
// MIDI hands over as `MIDIPort.manufacturer` with the permission the page already holds. No SysEx,
// no WebUSB picker, no endpoint, and no gateware -- which mattered, because the ECP5 has 559
// TRELLIS_COMB left of 24,288 and a router that only converges on a hand-picked seed. See
// `boards/tiliqua/gateware/build_id.py` for the two other descriptor fields that were rejected.
//
// Two sources, kept visibly apart because they answer different questions and often disagree:
//
//   * the boards themselves -- authoritative, per board, and silent on a Basys 3 or on a Tiliqua
//     carrying a bitstream older than the stamp.
//   * `firmware.json` -- what `boards/*/firmware/` holds in the commit this page was built from.
//     Generated by `scripts/build_firmware_json.py` out of the artefacts, never out of file
//     mtimes, which a fresh clone rewrites.
//
// Both name the commit they were built from, so the interesting comparison is free: same commit
// means the board is running what the repo ships.
let shipped = null;                        // firmware.json, fetched once and kept
async function shippedFirmware() {
  if (!shipped) {
    try { shipped = await (await fetch('firmware.json?' + VERSION)).json(); }
    catch (e) { shipped = { boards: [], error: String(e.message || e) }; }
  }
  return shipped;
}
const fwRow = (k, v, cls) =>
  `<div class="setrow"><span class="setk">${k}</span><span class="setv${cls ? ' ' + cls : ''}">${v}</span></div>`;
...
async function refreshFirmware() {
  ...
  if (!onboard || !ship) return;

  // Powered up: ask the transports, which hold the ports. Not powered: the silent probe, which
  // returns nothing until Web MIDI has been granted once -- and that is the honest answer, since
  ...
  if (!boards.length) {
    onboard.innerHTML = fwRow('On the board',
      'no board connected — press POWER, then reopen this panel', 'unset');
  } else {
    onboard.innerHTML = boards.map((b) => {
      // The key column is one narrow word wide, so the board number goes on the end rather than
...
function openSettings() { refreshSettings(); refreshOutputs(); refreshFirmware(); document.getElementById('setbox').classList.remove('hidden'); }
