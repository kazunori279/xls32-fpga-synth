...
# Two Tiliqua archives since M36 (#37): 24 voices is the formal build and 32 is kept as the
# experimental one. Listed formal-first, which is the order the panel renders.
#
# They are built from the same commit, so `manifest_tag` is the same string on both and the board's
# own USB stamp -- which is that commit -- cannot say which of the two is flashed. Nothing in the
# bitstream reports its voice count. The panel is written to say so rather than to guess; see the
# note beside `shipTiliqua` in webui/static/app.js.
TILIQUA = (
    {"key": "tiliqua-24", "tar": "boards/tiliqua/firmware/xls24-r5.tar.gz",
     "voices": 24, "slot": 7, "status": "formal"},
    {"key": "tiliqua-32", "tar": "boards/tiliqua/firmware/xls32-r5.tar.gz",
     "voices": 32, "slot": 6, "status": "experimental"},
)
...
def tiliqua(spec, commit):
    ...
    return {
        "board": "Tiliqua",
        # What the panel prints. `board` stays "Tiliqua" so the flashed-vs-shipped matching below
        # keeps working on it; this is the disambiguator, and it is the whole difference between
        # the two rows.
        "label": f"Tiliqua · {spec['voices']} voices",
        "voices": spec["voices"],
        "slot": spec["slot"],
        "status": spec["status"],
        "artefact": spec["tar"],
...
def generate():
    ...
    boards = [tiliqua(s, hashes[s["key"]]["built_from_commit"]) for s in TILIQUA]
    boards.append(basys3(hashes["basys3"]["built_from_commit"]))
    return {
...
def main():
    ...
    if not args.check:
        ...
        for b in fresh["boards"]:
            print(f"{b.get('label', b['board']):22} {b['built']} {b['tz'] or 'local'}  "
                  f"{b['commit']}  {b['sha256']}")
