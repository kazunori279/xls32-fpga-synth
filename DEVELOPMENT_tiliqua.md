...
- [Development history (Tiliqua)](#development-history-tiliqua)
  ...
  - [M36 — 24 voices, and the measurement that had been wrong for four milestones](#milestone-36--24-voices-and-the-measurement-that-had-been-wrong-for-four-milestones)
  - [M37 — the reduction finishes, and both boards get rebuilt](#milestone-37--the-reduction-finishes-and-both-boards-get-rebuilt)
...
## Milestone 36 — 24 voices, and the measurement that had been wrong for four milestones

**The question this milestone answers is not new.** [Risk 3](#what-is-left--m32-and-the-risk-register)
has been open since M21: the `clk` domain is 60 MHz, fixed by ULPI, and the design does not close
it. At 32 voices nextpnr routes to **46.35 MHz**, so the bitstream only works if the silicon is 29 %
faster than the model. It is, on this desk. It was not on one of the two modules the vendor tried,
which is [#3](https://github.com/kazunori279/xls32-fpga-synth/issues/3).

The obvious lever is polyphony, and this repo had ruled it out. `ARCHITECTURE_tiliqua.md` §E1 had
carried a ladder since M24 — 32 → 23,792 cells, 24 → 19,631, 16 → 19,222 — with the reading that 24
voices bought 4,161 cells and only +5.8 MHz, and that 16 bought nothing at all. "Measured and dead"
was in `docs/TODO.md`. Four milestones of timing work went around it.

**The ladder was measuring a pruned netlist.** `spike/voices_variant.py` rewrites a throwaway copy
of `core/synth.x` to N voices by regex, asserting a count on every rule so a rule that stops
matching is loud. But `rotate_in`'s tail writeback had **no rule**:
...
fn rotate_in(v: Voice[32], tail: Voice) -> Voice[32] {
    ...
    update(shifted, u32:31, tail)      // <- the loop bound above had a rule. This did not.
}
```

At N=24 the array became `Voice[24]` and the loop bound became `u32:23`, but the last line kept its
literal `u32:31`. **`update` with an out-of-range index in DSLX is a no-op.** The rotated ring is
therefore returned without the incoming voice, no voice ever enters, every slot is unreachable
state, and yosys deletes the register file.

Every consequence of that points the wrong way. The area drops — that is the 4,161 cells. Fmax
rises, because the critical path went with it. Nothing errors, at any layer: DSLX type-checks, XLS
codegens, yosys warns about nothing it does not warn about anyway, nextpnr routes it. The only
symptom is that the bitstream plays silence, and nobody had ever flashed one, because the numbers
said not to bother.

**How it was caught, which was luck plus one habit.** Re-running the ladder on today's tree to
re-check the fallback before writing to the vendor, the 32-voice row moved 23,792 → 24,023 (+231,
consistent with four milestones of drift) and the 24-voice row moved 19,631 → 22,796 (+3,165, not
consistent with anything). Two rows from the same generator disagreeing about how much a year of
commits costs is not a result, it is a bug report. The flip-flop count settled it: +2,475 FF is a
register file reappearing, not logic accreting. The habit was reading the generated file rather than
trusting the assertion counts — `grep -n rotate_in` on the 24-voice output showed `u32:31` against a
`Voice[24]` in one line.

**Why the file's own identity check could not have caught it.** `voices_variant.py --voices 32` is
required to reproduce `core/synth.x` byte for byte, and it does, and that test is worth having. It
cannot find a missing rule: at N=32 every substitution is a no-op *by construction*, so the output
is identical whether a site has a rule or not. The check proves the rules are harmless. Completeness
is a different property and needs a reduced output to test against. Two rules were added, anchored
on the whole `update(shifted, u32:31, tail)` call so they cannot eat the loop bound above.

**The corrected curve** — same tree, same day, converged runs only (`overused=0`, "Routing
complete", a `.config` on disk; see the trap below):

| voices | TRELLIS_COMB | | FF | `clk` Fmax | converged |
|---|---|---|---|---|---|
| 32 | 24,023 | 98.9 % | 13,223 | 46.35 MHz | 3 of 6 seeds |
| 28 | 23,358 | 96.2 % | 12,660 | 48.85 MHz | 7 of 8 |
| **24** | **22,796** | **93.9 %** | **11,931** | **55.48 MHz** | 4 of 5 |

Not two rungs and not a cliff: a slope, and one worth walking down. 24 voices costs 1,227 cells and
buys **9.13 MHz**, which takes the bet against the silicon from 29 % to 8 %. 16 voices was left
unmeasured — every prior 16-voice number is contaminated
...
**Two traps, and both of them read high.** First, a nextpnr run that never converges still writes a
timing report, and the Fmax in it is meaningless. Second — subtler, and it caught me mid-sweep — a
run *still in progress* has already written the **post-placement estimate**, at `top.tim` line ~299,
and it is not the routed result at line ~1112. `--router2-tmg-ripup` read 49.19 MHz that way and
finished at 47.39. No number off this flow means anything until the log says "Routing complete" and
a `.config` exists.

**The seed is load-bearing and does not transfer.** On the 24-voice netlist, seeds 1/4/5/6 gave
51.17 / 55.48 / 48.45 / 54.03 MHz — a 7 MHz spread on identical input. Seed 5 is the best of the
32-voice draw and among the worst of the 24-voice one. `build.sh` pinned 4 for 24 and 5 for 32, and
any netlist change costs a fresh sweep. This is also why the build stamp matters: `build_id.py`
bakes `<utc>-<commit>` into a ROM, a dirty tree appends `-dirty`, and those six characters change
the netlist's width and re-draw the lottery. M37 measured what that is worth and it is not a
rounding error; see below.

**What shipped.** 24 voices is now the default and the formal bitstream — `boards/tiliqua/build.sh`
defaults to `VOICES=24`, `xls24-r5.tar.gz` goes to **slot 7**, and the 32-voice archive stays as
`xls32-r5.tar.gz` on **slot 6**, relabelled experimental rather than deleted
([#37](https://github.com/kazunori279/xls32-fpga-synth/issues/37)). On hardware the 24-voice build
grades **99.8/100 (A+)**, 174 pass / 1 warn / 0 fail, frame gaps 0.00 % over 175 captures, pitch
440.01 Hz (+0.1 cents) — the same grade as 32, because voice count does not touch sample rate
(codec backpressure sets that, `gateware/xls_core.py`) and the suite's densest case only asks for 32
simultaneous notes, which 24 voices answers by stealing cleanly.

One cosmetic thing did not survive the reduction: `gateware/viz.py` fixes the visualiser grid at 8×4
and never sees `VOICES`, so the 24-voice build's bottom row of tiles is drawn from slots that are
never written and sits at the idle floor forever
([#40](https://github.com/kazunori279/xls32-fpga-synth/issues/40)). It is eight dim tiles and no
audio consequence, and fixing it means a netlist change, a fresh seed sweep and another hardware
run — so it waits for the next time the netlist moves anyway.

**The lesson.** An optimisation that makes a design *smaller and faster* is the one nobody audits,
which makes silent-pruning the most expensive class of bug a generator can have. The specific
mechanism — an out-of-range `update` being a legal no-op rather than an error — is a DSLX property
worth knowing; the general one is that a code generator's assertions can only cover the rules it
has, so its real test is reading the output it produces at the setting it is used at. This one had
been asserting confidently and correctly about the wrong thing for four milestones, and it changed
a roadmap.

## Milestone 37 — the reduction finishes, and both boards get rebuilt

M36 changed the voice count and stopped. This milestone is the cleanup that turns 24 voices from a
number in a build script into the thing the boards actually are, and it is the first one in a while
that touches the Basys 3.

**Three things the 24-voice build was still lying about.** The count lives in DSLX — `voices_variant.py`
rewrites the ring length before XLS compiles anything — but the Python side had written it down
twice more as the literal 32. `viz.py` fixed the grid at 8×4, so the bottom row of the 24-voice
build was drawn from four slots nothing ever wrote and sat at the idle floor forever
([#40](https://github.com/kazunori279/xls32-fpga-synth/issues/40)); `xls_core.py` announced the
bitstream to the bootloader as "XLS32 synth", which is the project's name and not the voice count
([#43](https://github.com/kazunori279/xls32-fpga-synth/issues/43)). `gateware/voices.py` is now the
one place: it reads `$VOICES`, which `build.sh` exports, and picks the grid from it — 4×4, 6×4 or
8×4, all of which divide 720 exactly.

**Making the grid a parameter cost a multiplier this die does not have.** The tile index was
`Cat(col, row)`, free while COLS was 8 because a shift by a power of two is a renaming of wires. At
24 voices COLS is 6, and the obvious `row * COLS + col` makes yosys emit a `$mul` — against a
constant — which becomes a **MULT18X18D**. All 28 of this die's multipliers are already spoken for
by the engine ([#6](https://github.com/kazunori279/xls32-fpga-synth/issues/6)), so the build dies in
the placer with "no BELs remaining" rather than anywhere informative. Summing the set bits of COLS
is what the multiply would have decomposed into anyway: `(row << 1) + (row << 2)`, two adds on a
two-bit operand. The column counter needed an explicit wrap for the same reason — with COLS at 6, a
bare `col + 1` leaves 6 sitting in a three-bit signal for the rest of the line.

**The seed draw, and how it nearly shipped the wrong one.** `--router2-tmg-ripup` went on
([#39](https://github.com/kazunori279/xls32-fpga-synth/issues/39)) — it had been ruled out during
M36 on a measurement that was actually a mid-run post-placement estimate, and that reason expired
the moment the tile grid changed the netlist. On the netlist that shipped, all six seeds converged
where M36 had four of five, and read:

| seed | 1 | 2 | **3** | 4 | 5 | 6 |
|---|---|---|---|---|---|---|
| `clk` Fmax | 52.50 | 54.06 | **54.30** | 49.12 | 51.40 | 53.07 |

Seed 3 wins by 0.24 MHz over seed 2. The spread across the six is 5.2 MHz with no gap worth calling
a winner's margin, so the pin is "the best measured" and not a property of the design.

Seed 4 is the one worth the warning, because measuring it is how this sweep first got the wrong
answer. It read **56.33 MHz** — best of the draw by two megahertz — on a build whose tree was dirty.
`9fca1a0-dirty` is six characters longer than a clean seven-digit sha, the stamp is a ROM, and its
*length* moves the netlist: **22,745 cells against 22,985** for the clean one. Re-measured on the
netlist that would actually ship, seed 4 is 49.12 — the *worst* of the six. Two lessons, and the
second is the general one. A sweep has to run on the netlist that will ship, and "dirty tree" counts
as a different netlist. And seed rankings do not transfer, not even from the same seed on a netlist
240 cells away.

**What shipped on the Tiliqua.** `xls24-9976c4e-r5.tar.gz`: **54.30 MHz**, 22,985 TRELLIS_COMB
(94.6 %), 11,931 FF. On the module it grades **99.8/100 (A+)** — 174 pass / 1 warn / 0 fail over the
175-case suite, frame gaps 0.00 % across 175 captures, audio clock 12.288 MHz, all seven stress
cases at zero glitches. The one warn is the `filter_sweep` centroid,
[#7](https://github.com/kazunori279/xls32-fpga-synth/issues/7), which the 32-voice build shares. The
grid draws 6 × 4 and the bootloader's slot list reads
`24-voice polysynth, 4 MIDI parts. TRS or USB MIDI in, audio out`, both confirmed on hardware. Note
that 94.6 % against M36's 93.9 % is not the reduction regressing: it is the same design at the same
voice count, 189 cells larger for the wrap logic and the parameterised strings.

**The Basys 3 had been carrying a two-week-old blob** ([#41](https://github.com/kazunori279/xls32-fpga-synth/issues/41)).
Both of M35's DC fixes are `core/synth.x` changes and therefore belong to both boards, and only the
Tiliqua had been rebuilt. The entry in `docs/TODO.md` had predicted its own cost wrongly — it blamed
the x86 machine and the desk space, and the real cost turned out to be that two weeks of engine
changes had gone unbuilt and one of them no longer closed timing.

The regression was the `ardy` handshake. `ardy` is `_audio_out_rdy`, and inside the XLS-generated
engine it feeds the combinational backpressure chain that runs backwards through all 48 stages,
reaching the CE and R pins of essentially every register in `eng`. Those registers only move on `ce`
— a divide-by-three enable, 30 ns — but `top.v` was setting `ardy` on *any* clock, which makes it a
genuine single-cycle path that no constraint can honestly declare multicycle. So the source had to
change before the timing script could. Two DSP-related exceptions went in alongside it, and which of
the three bought the margin was not isolated: one build each was not worth the VM time. Worth
knowing for the next person who writes a name-matched exception, though — Vivado had absorbed
`revwetL_reg` into the DSP as `rwetL_p`'s B input register, so the cell disappeared from
`get_cells -filter {IS_SEQUENTIAL && NAME =~ *revwetL_reg*}` and its exception went with it.

The rebuild closed at **+1.322 ns worst setup slack, zero failing endpoints** against M34's
+0.276 ns, at 10,568 LUTs (50.8 %) and 17,978 registers (43.2 %) — within a hundred cells of the old
blob either way. On the board, `verify.sh` returned **PASS: 4/4 notes**: an A major 7 came back at
438 / 554 / 658 / 830 Hz against nominal 440.0 / 554.4 / 659.3 / 830.6, all four inside the bin the
check can resolve, over 80,163 samples with the frame phase holding across all 40 windows. The
scoped `known_stale` waiver that had been sitting on the Basys 3 record since M35 is gone with it.

**What is still stale, deliberately.** The 32-voice archive was not rebuilt against any of this
([#46](https://github.com/kazunori279/xls32-fpga-synth/issues/46)). At 98.9 % occupancy a rebuild is
...
**The 24-voice archive was rebuilt in the same pass**, for the record and not for the bitstream:
M39 touched `build.sh` and `top.py`, which is enough to report it stale, though `XLS32_SPLIT_CLOCKS`
is off by default and neither edit reaches the netlist. It came back at **22,722 cells and 56.63
MHz** — the shipped numbers to the cell and the megahertz — and graded **99.8/100 (A+), 174/1/0**
with `filter_sweep` at 83.4. So no published 24-voice figure moves, and the two archives are now
recorded against the same commit and the same SDK checkout. A rebuild that reproduces its own
numbers exactly is the cheapest evidence available that the seed pin and the build script still
mean what they say.

Both suites agreed on the thing M40 had just fixed: **0 frames missing across 350 captures**.

**2026-08-25: die #2 ran the 24-voice archive too, from flash.** The 32-voice run above left slot 7
already carrying `xls24-r5.tar.gz`, so there was nothing to reflash — `pdm run flash status` reads
`XLS24` / tag `3cc9951` there against `XLS32` / `b9b2345` on slot 6, both matching the committed
archives, and the module was booted from slot 7 with the stamp `2026-08-24T08:48Z-3cc9951` to prove
it. `check_loop.py` read 440.01 Hz (+0.0 cents) and 0.000 % gaps; the suite returned **99.8/100
(A+), 174 pass / 1 warn / 0 fail**, gaps 0.00 %, audio clock 12.288 MHz (spread 11.5 kHz), 0 frames
missing over 0 of 175 captures, `filter_sweep` warning at 82.3. Die #1 gives the same archive the
same 99.8 with the same lone warn, so the two agree to the tenth and the 24-voice result no longer
rests on one module.

That was the condition the webflasher swap had been waiting on, and it is now met — see
`docs/TODO.md` for why the swap is still a judgement call rather than an upgrade.
